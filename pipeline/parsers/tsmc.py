...
def parse_financial_statement(text: str) -> dict:
    ...
    def quarterly_ntd(scope: str, label: str) -> float | None:
        m = re.search(
            re.escape(label) + r"\s*\n((?:\s*\$?\s*\(?[\d,]+\)?\s*\n?){4,5})",
            scope, re.I,
        )
    ...
    ocf = quarterly_ntd(text, "Net Cash Generated by Operating Activities")
    if ocf is not None:
        kpis["operating_cash_flow_ntd_mn"] = ocf

    # "Property, Plant and Equipment" also appears as a balance-sheet asset
    # row earlier in this document (different shape, not what we want) -
    # scope the search to the investing-activities section, where it's the
    # capex line ("Acquisitions of: / Property, Plant and Equipment / ...").
    investing_section = text.split("Acquisitions of:", 1)
    capex = quarterly_ntd(investing_section[1], "Property, Plant and Equipment") if len(investing_section) > 1 else None
    if capex is not None:
        kpis["capex_ntd_mn"] = -abs(capex)  # acquisitions of PP&E is an outflow
    if ocf is not None and capex is not None:
        kpis["free_cash_flow_ntd_mn"] = round(ocf - abs(capex), 1)
